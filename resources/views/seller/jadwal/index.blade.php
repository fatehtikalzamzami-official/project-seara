    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-green {
            ...
            box-shadow: 0 4px 12px rgba(61,186,126,.25); text-decoration: none;
        }
        ...
        .upcoming-banner .ub-text strong { font-size: 15px; font-weight: 900; display: block; }
        .upcoming-banner .ub-text span   { font-size: 12px; color: rgba(255,255,255,.7); font-weight: 600; }
        .upcoming-banner .ub-count       { margin-left: auto; font-size: 32px; font-weight: 900; color: var(--green-light); line-height: 1; }

        /* ── Stats Bar ── */
        .stats-bar { display: grid; grid-template-columns: repeat(5,1fr); gap: 12px; margin-bottom: 22px; }
        .stat-card {
            background: white; border: 1px solid var(--border);
            border-radius: var(--r); padding: 16px 18px; cursor: pointer; transition: all .2s;
        }
        .stat-card:hover, .stat-card.active { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: var(--green-main); }
        .stat-card.active .stat-value { color: var(--green-main); }
        ...
        .search-box {
            ...
            padding: 8px 14px; flex: 1; min-width: 200px; max-width: 300px;
        }
        ...
        /* ── View Toggle ── */
        .view-toggle { display: flex; gap: 4px; background: white; border: 1.5px solid var(--border); border-radius: 10px; padding: 4px; margin-left: auto; }
        .vt-btn { width: 32px; height: 32px; border: none; border-radius: 7px; cursor: pointer; background: transparent; color: var(--text-muted); font-size: 13px; transition: all .18s; display: flex; align-items: center; justify-content: center; }
        .vt-btn.active { background: var(--green-pale); color: var(--green-dark); }

        /* ── Schedule Grid ── */
        .schedule-grid { display: grid; grid-template-columns: repeat(auto-fill,minmax(300px,1fr)); gap: 16px; }
        .sc-card { background: white; border: 1px solid var(--border); border-radius: var(--r); overflow: hidden; transition: all .2s; position: relative; }
        .sc-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-md); border-color: var(--green-main); }
        .sc-card::before { content:''; position:absolute; left:0; top:0; bottom:0; width:4px; }
        .sc-card[data-status="direncanakan"]::before  { background: var(--purple); }
        .sc-card[data-status="sedang_tumbuh"]::before { background: var(--green-main); }
        .sc-card[data-status="siap_panen"]::before    { background: var(--yellow); }
        .sc-card[data-status="selesai"]::before       { background: var(--blue); }
        .sc-card[data-status="gagal"]::before         { background: var(--accent); }

        .sc-head { padding: 14px 16px 10px 20px; display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; }
        .sc-emoji { font-size: 36px; line-height: 1; flex-shrink: 0; }
        .sc-head-info { flex: 1; }
        .sc-category { font-size: 10px; font-weight: 800; color: var(--green-main); text-transform: uppercase; letter-spacing: .8px; margin-bottom: 2px; }
        .sc-name { font-size: 16px; font-weight: 900; color: var(--text-dark); line-height: 1.25; }
        .sc-status-badge { font-size: 10px; font-weight: 800; padding: 4px 10px; border-radius: 20px; display: flex; align-items: center; gap: 4px; white-space: nowrap; flex-shrink: 0; }

        .sc-body { padding: 0 16px 4px 20px; }
        .sc-countdown { display: inline-flex; align-items: center; gap: 6px; background: var(--green-pale); border-radius: 20px; padding: 5px 12px; margin-bottom: 12px; font-size: 12px; font-weight: 800; color: var(--green-dark); }
        .sc-countdown.urgent  { background: #fef3c7; color: #92400e; }
        .sc-countdown.past    { background: #f0fdf4; color: #166534; }
        .sc-countdown.overdue { background: #fef2f2; color: #b91c1c; }

        .sc-info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 12px; }
        .sc-info-item { display: flex; flex-direction: column; gap: 1px; }
        .sc-info-label { font-size: 9px; font-weight: 800; text-transform: uppercase; letter-spacing: .8px; color: var(--text-muted); }
        .sc-info-value { font-size: 12px; font-weight: 700; color: var(--text-mid); }

        .sc-tags { display: flex; gap: 5px; flex-wrap: wrap; margin-bottom: 12px; }
        .sc-tag { font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 20px; }
        .sc-tag.organic { background: var(--green-pale); color: var(--green-dark); }
        .sc-tag.metode  { background: #eff6ff; color: #1d4ed8; }

        .sc-notes { font-size: 11px; color: var(--text-muted); font-weight: 600; background: #fafafa; border-radius: 8px; padding: 8px 10px; margin-bottom: 12px; border-left: 3px solid var(--border); line-height: 1.5; }

        .sc-footer { padding: 10px 16px 14px 20px; display: flex; gap: 8px; border-top: 1px solid var(--border); }
        .sc-btn { flex: 1; padding: 8px; border-radius: 8px; font-family: 'Nunito',sans-serif; font-size: 12px; font-weight: 800; cursor: pointer; transition: all .2s; border: none; display: flex; align-items: center; justify-content: center; gap: 5px; }
        .sc-btn.edit   { background: var(--green-pale); color: var(--green-dark); }
        .sc-btn.edit:hover   { background: #bbf7d0; }
        .sc-btn.delete { background: #fef2f2; color: #b91c1c; }
        .sc-btn.delete:hover { background: #fecaca; }

        .status-select-inline { font-size: 11px; font-weight: 700; border: 1.5px solid var(--border); border-radius: 8px; padding: 5px 8px; font-family: 'Nunito',sans-serif; cursor: pointer; background: white; color: var(--text-mid); outline: none; }
        .status-select-inline:focus { border-color: var(--green-main); }

        /* ── Progress bar ── */
        .sc-progress { margin-bottom: 12px; }
        .sc-progress-label { display: flex; justify-content: space-between; font-size: 10px; font-weight: 700; color: var(--text-muted); margin-bottom: 4px; }
        .sc-progress-bar  { height: 6px; background: var(--border); border-radius: 10px; overflow: hidden; }
        .sc-progress-fill { height: 100%; border-radius: 10px; background: linear-gradient(90deg, var(--green-main), var(--green-mid)); transition: width .5s ease; }

        /* ── List view ── */
        .schedule-list { display: none; flex-direction: column; gap: 10px; }
        ...
        .sl-row::before { content:''; position:absolute; left:0; top:0; bottom:0; width:4px; }
        .sl-row[data-status="direncanakan"]::before  { background: var(--purple); }
        .sl-row[data-status="sedang_tumbuh"]::before { background: var(--green-main); }
        .sl-row[data-status="siap_panen"]::before    { background: var(--yellow); }
        .sl-row[data-status="selesai"]::before       { background: var(--blue); }
        .sl-row[data-status="gagal"]::before         { background: var(--accent); }
        .sl-row:hover { transform: translateX(2px); box-shadow: var(--shadow-sm); border-color: var(--green-main); }
        .sl-emoji { font-size: 28px; flex-shrink: 0; width: 44px; text-align: center; }
        .sl-main  { flex: 1; min-width: 0; }
        .sl-name  { font-size: 14px; font-weight: 900; color: var(--text-dark); margin-bottom: 2px; }
        .sl-meta  { display: flex; gap: 14px; flex-wrap: wrap; }
        .sl-meta span { font-size: 11px; font-weight: 700; color: var(--text-muted); display: flex; align-items: center; gap: 4px; }
        .sl-right { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
        ...
        /* ════════════════════════════════════════════════════════
           MODAL — ukuran & layout diperbaiki
           ════════════════════════════════════════════════════════ */
        .modal-overlay {
        ...
        .modal-head h2 { font-size: 16px; font-weight: 900; color: var(--text-dark); display: flex; align-items: center; gap: 9px; }

        .modal-close {
            background: #f1f5f1; border: none; cursor: pointer; font-size: 14px;
            color: var(--text-muted); width: 32px; height: 32px; border-radius: 50%;
            display: flex; align-items: center; justify-content: center; transition: all .18s;
        }
        .modal-close:hover { background: #e2ece2; color: var(--text-dark); }
        ...
        .form-input:focus, .form-select:focus, .form-textarea:focus {
            ...
            box-shadow: 0 0 0 3px rgba(45,134,83,.10);
        }
        ...
        .form-hint { font-size: 11px; color: var(--text-muted); font-weight: 600; margin-top: 4px; display: block; }
        ...
        .toggle-wrap:hover { background: #dcfce7; }
        .toggle-wrap label {
...
<body>
        ...
        <main class="seller-main">
            ...
            <div class="page-header anim-1">
                <div class="page-header-left">
                    <h1>Jadwal Panen <i class="fa-solid fa-calendar-days" style="font-size:22px;color:var(--green-main)"></i></h1>
            ...
            @else
                    ...
                    @foreach($schedules as $s)
                        @php
                            $sisaHari = (int) now()->startOfDay()->diffInDays(
                                \Carbon\Carbon::parse($s->estimasi_panen)->startOfDay(), false
                            );
                            $countdownClass = $sisaHari < 0 ? 'overdue' : ($sisaHari <= 7 ? 'urgent' : '');
                            ...
                            $progress = 0;
                            if ($s->tanggal_tanam) {
                                $totalDays  = \Carbon\Carbon::parse($s->tanggal_tanam)->diffInDays(\Carbon\Carbon::parse($s->estimasi_panen));
                        ...
                        <div class="sc-card" data-status="{{ $s->status }}"
                            data-name="{{ strtolower($s->nama_tanaman) }}"
                            data-category="{{ $s->category->name ?? '' }}"
                            data-metode="{{ $s->metode_tanam ?? '' }}">
                            ...
                            <div class="sc-head">
                                ...
                                <span class="sc-status-badge" style="background:{{ $badge[0] }};color:{{ $badge[1] }}">
                                    <i class="fa-solid {{ $badge[3] }}"></i> {{ $badge[2] }}
                                </span>
                            </div>
                                ...
                                @if($s->tanggal_tanam && !in_array($s->status, ['selesai','gagal']))
                                    <div class="sc-progress">
                                        <div class="sc-progress-label">
                                            <span><i class="fa-solid fa-seedling"></i> {{ \Carbon\Carbon::parse($s->tanggal_tanam)->isoFormat('D MMM') }}</span>
                                            <span>{{ $progress }}% masa tanam</span>
                                            <span><i class="fa-solid fa-tractor"></i> {{ \Carbon\Carbon::parse($s->estimasi_panen)->isoFormat('D MMM') }}</span>
                                        </div>
                                ...
                                <div class="sc-info-grid">
                                    @if($s->estimasi_kuantitas)
                                        <div class="sc-info-item">
                                            <span class="sc-info-label"><i class="fa-solid fa-scale-balanced"></i> Est. Kuantitas</span>
                                            <span class="sc-info-value">{{ number_format($s->estimasi_kuantitas,1) }} {{ $s->satuan ?? '' }}</span>
                                        </div>
                                    @endif
                                    @if($s->kebun_lokasi)
                                        <div class="sc-info-item">
                                    ...
                                    @if($s->tanggal_tanam)
                                        <div class="sc-info-item">
                                            ...
                                            <span class="sc-info-value">{{ \Carbon\Carbon::parse($s->tanggal_tanam)->isoFormat('D MMM YYYY') }}</span>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="sc-footer">
                                ...
                                <form method="POST" action="{{ route('seller.jadwal.status', $s->id) }}" style="flex:1">
                                    ...
                                    <select name="status" class="status-select-inline" style="width:100%"
                                        ...
                                        <option value="direncanakan"  {{ $s->status==='direncanakan'  ? 'selected':'' }}>📅 Direncanakan</option>
                                        <option value="sedang_tumbuh" {{ $s->status==='sedang_tumbuh' ? 'selected':'' }}>🌱 Sedang Tumbuh</option>
                                        <option value="siap_panen"    {{ $s->status==='siap_panen'    ? 'selected':'' }}>🚜 Siap Panen</option>
                ...
                <div class="schedule-list anim-3" id="scheduleList">
                    @foreach($schedules as $s)
                        ...
                        @endphp
                        <div class="sl-row" data-status="{{ $s->status }}"
                            data-name="{{ strtolower($s->nama_tanaman) }}"
                            data-category="{{ $s->category->name ?? '' }}"
                            data-metode="{{ $s->metode_tanam ?? '' }}">
                            ...
                            <div class="sl-main">
                                ...
                                <div class="sl-meta">
                                    <span><i class="fa-solid fa-tractor"></i> Panen {{ \Carbon\Carbon::parse($s->estimasi_panen)->isoFormat('D MMM YYYY') }}</span>
                                    @if($s->kebun_lokasi)<span><i class="fa-solid fa-location-dot"></i> {{ $s->kebun_lokasi }}</span>@endif
                                    @if($s->estimasi_kuantitas)<span><i class="fa-solid fa-scale-balanced"></i> {{ $s->estimasi_kuantitas }} {{ $s->satuan }}</span>@endif
                                    <span>{{ $sisaHari >= 0 ? $sisaHari.' hari lagi' : abs($sisaHari).' hari telat' }}</span>
                                </div>
                            </div>
                            <div class="sl-right">
                                <span class="sc-status-badge" style="background:{{ $badge[0] }};color:{{ $badge[1] }}">{{ $badge[2] }}</span>
    ...
    <div class="modal-overlay" id="modalOverlay" onclick="closeModalOnBg(event)">
        <div class="modal">
            ...
            <div class="modal-head">
                <h2 id="modalTitle">
                    <i class="fa-solid fa-plus" style="color:var(--green-main)"></i>
                    Tambah Jadwal Panen 1
                </h2>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nama_tanaman" id="namaTanaman" class="form-input"
                                placeholder="cth: Tomat, Cabai Rawit, Bayam…" required>
                        </div>
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="metode_tanam" id="metodeTanamInput" class="form-select">
                                    ...
                                    <option value="hidroponik">💧 Hidroponik</option>
                                    <option value="konvensional">🌾 Konvensional</option>
                                    <option value="permakultur">♻️ Permakultur</option>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <span class="form-hint">Opsional — untuk menghitung progres tumbuh</span>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">Estimasi Kuantitas</label>
                                <input type="number" name="estimasi_kuantitas" id="estimasiKuantitas"
                                    class="form-input" placeholder="cth: 100" min="0" step="0.1">
                            </div>
                            <div class="form-group">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="kebun_lokasi" id="kebunLokasiInput" class="form-input"
                                    placeholder="cth: Blok A, Desa Cikaret, Cianjur">
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="catatan" id="catatanInput" class="form-textarea"
                                placeholder="cth: bibit varietas unggul, perkiraan cuaca baik, dll…"></textarea>
                        </div>
                        ...
                        <div class="toggle-wrap" onclick="document.getElementById('isOrganicInput').click()">
                            <input type="checkbox" name="is_organic" id="isOrganicInput" value="1"
                                style="width:17px;height:17px;accent-color:var(--green-main);flex-shrink:0"
                                onclick="event.stopPropagation()">
                            <label for="isOrganicInput">
                                <i class="fa-solid fa-leaf" style="color:var(--green-main)"></i>
                                Tanaman ini ditanam secara <strong>organik</strong> (tanpa pestisida / bahan kimia)
                            </label>
                        </div>
                    </div>
    ...
    <div class="modal-overlay" id="deleteOverlay" onclick="closeDeleteOnBg(event)">
        <div class="modal" style="max-width:420px">
            ...
            <div class="modal-body" style="padding:20px 24px">
                <p style="font-size:14px;font-weight:600;color:var(--text-mid);line-height:1.7">
                    Apakah Anda yakin ingin menghapus jadwal panen
                    <strong id="deleteJadwalName" style="color:var(--text-dark)"></strong>?<br>
            ...
            <div class="modal-foot">
                ...
                <form id="deleteForm" method="POST" style="margin:0">
                    @csrf @method('DELETE')
                    <button type="submit"
                        style="padding:9px 20px;background:#b91c1c;border:none;border-radius:10px;
                               font-family:'Nunito',sans-serif;font-weight:800;font-size:13px;
    ...
    <script>
        ...
        (function () {
            if (localStorage.getItem('jadwalView') === 'list') setView('list');
        })();
        ...
        function openEditModal(id, nama, categoryId, tanggalTanam, estimasiPanen,
                               kuantitas, satuan, lokasi, metode, status,
                               catatan, luas, isOrganic) {
            document.getElementById('modalTitle').innerHTML =
            ...
            document.getElementById('statusInput').value       = status        || 'direncanakan';
        ...
        function resetForm() {
            ['namaTanaman','tanggalTanam','estimasiPanen',
             'estimasiKuantitas','kebunLokasiInput','catatanInput','luasLahan']
